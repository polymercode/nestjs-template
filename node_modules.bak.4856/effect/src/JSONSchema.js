...
exports.fromAST = exports.make = void 0;
exports.getMetaSchemaUri = getMetaSchemaUri;
const Arr = __importStar(require("./Array.js"));
const errors_ = __importStar(require("./internal/schema/errors.js"));
const schemaId_ = __importStar(require("./internal/schema/schemaId.js"));
const Option = __importStar(require("./Option.js"));
const ParseResult = __importStar(require("./ParseResult.js"));
const Predicate = __importStar(require("./Predicate.js"));
const Record = __importStar(require("./Record.js"));
const AST = __importStar(require("./SchemaAST.js"));
const make = (schema, options) => {
    ...
    const ast = AST.isTransformation(schema.ast) && isParseJsonTransformation(schema.ast.from)
        ? schema.ast.to
        : schema.ast;
    const jsonSchema = (0, exports.fromAST)(ast, {
        definitions,
        target
    });
    const out = {
...
function getRawDefault(annotated) {
    if (annotated !== undefined)
        return AST.getDefaultAnnotation(annotated);
    return Option.none();
}
...
const mergeRefinements = (from, jsonSchema, ast) => {
    const out = { ...from, ...getJsonSchemaAnnotations(ast), ...jsonSchema };
    out.allOf ??= [];
...
function addAnnotations(jsonSchema, annotations) {
    ...
    if ("$ref" in jsonSchema) {
        return { allOf: [jsonSchema], ...annotations };
    }
...
function compactUnion(members) {
    ...
    for (const m of members) {
        if (isNeverWithoutCustomAnnotations(m))
            continue;
        if (isAny(m) || isUnknown(m) || isVoid(m))
            return [m];
